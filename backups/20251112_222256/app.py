...
try:
    ...
    print(f"⚠️ Vector store error (will work without RAG): {e}")
    vectorstore = None
...
if vectorstore:
    ...
    retriever = None
    qa_chain = None
    print("⚠️ Vector store not available - chatbot will work without RAG")
...
@app.route("/demo")
def demo_chatbox():
    """Demo page showing the floating chatbox widget"""
...
def chat():
    try:
        ...
        if qa_chain and hasattr(llm, 'invoke'):
            # For proper LangChain LLMs with QA chain (with RAG)
            response = qa_chain.invoke({"query": enhanced_query})
            reply = response["result"].strip().replace("\n", "<br>")
        elif hasattr(llm, 'invoke'):
            # LLM available but no RAG - direct response
            try:
                if retriever and safe_retriever_check():
                    # Try manual RAG
                    docs = retriever.get_relevant_documents(user_input)
                    context = "\n\n".join(doc.page_content for doc in docs)
                    full_prompt = template_text.format(context=context, question=user_input)
                else:
                    # No RAG - just use LLM directly
                    full_prompt = f"You are Bob AI, a friendly assistant. Help {name} with: {user_input}"
                
                full_prompt += f"\n\nAlways respond in HTML format (use <br> for line breaks). Be helpful and friendly."
        ...
        # Log the conversation (disabled - no database)
